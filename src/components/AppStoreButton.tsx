...
export function AppStoreButton({ platform, className = '', outline = false }: AppStoreButtonProps) {
  ...
  const baseClasses = "relative flex items-center gap-3 px-6 py-3 rounded-xl transition-all duration-300 transform hover:scale-105 active:scale-95 overflow-hidden opacity-90 cursor-default";
  const colorClasses = outline 
    ? "border-2 border-white/20 hover:border-primary-neon text-white hover:bg-white/5" 
    : "bg-white text-black hover:bg-gray-200";

  return (
    <a href="#" onClick={(e) => e.preventDefault()} className={`${baseClasses} ${colorClasses} ${className}`}>
      {/* Coming Soon Overlay */}
